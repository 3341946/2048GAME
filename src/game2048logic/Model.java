...
public class Model {
    ...
    public boolean maxTileExists() {
        ...
        for (int x = 0; x < board.size(); x += 1) {
            for (int y = 0; y < board.size(); y += 1) {
                if(board.tile(x,y)==null)continue;
                if(board.tile(x,y).value()==MAX_PIECE)return true;
            }
        }
        return false;
    }
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x = 0; x < board.size(); x += 1) {
            for (int y = 0; y < board.size(); y += 1) {
                if(board.tile(x,y)==null)return true;
                if(x > 0){
                    if(board.tile(x-1,y)==null)return true;
                    if(board.tile(x-1,y).value()==board.tile(x,y).value())return true;
                }
                if(x < board.size() - 1){
                    if(board.tile(x+1,y)==null)return true;
                    if(board.tile(x+1,y).value()==board.tile(x,y).value())return true;
                ...
                if(y > 0){
                    if(board.tile(x,y-1)==null)return true;
                    if(board.tile(x,y-1).value()==board.tile(x,y).value())return true;
                }
                if(y < board.size() - 1){
                    if(board.tile(x,y+1)==null)return true;
                    if(board.tile(x,y+1).value()==board.tile(x,y).value())return true;
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        for(int i = board.size() - 2; i >= y; i--) {
            Tile currTile = board.tile(x, i);
            if(currTile==null)continue;
            if(board.tile(x,targetY)==null)
                board.move(x,targetY,currTile);
            else if (currTile.value()==board.tile(x,targetY).value()){
                board.move(x,targetY,currTile);
                score+=currTile.value()*2;
                targetY--;
            } else {
                if(i!=targetY-1)board.move(x,targetY-1,currTile);
                targetY--;
            }
        }
    ...
    public void tilt(Side side) {
        ...
        for (int x = 0; x < board.size(); x += 1) moveTileUpAsFarAsPossible(x,0);
